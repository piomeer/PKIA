...
### 3.2 Memory Timestamp Fields

`created_at` 和 `updated_at` 是 Memory_Node 的时间维度字段，用于审计、版本追溯和活跃度判断。

| 字段 | 类型 | 必需 | 说明 |
|------|------|:----:|------|
| `created_at` | datetime | 否 | Memory_Node 首次创建时间。创建后永不修改。用于审计和项目演化分析。 |
| `updated_at` | datetime | 否 | 最近一次状态变更时间。在 Reinforce / Supersede / Status Change 时更新。用于活跃度判断。 |

**约束**:
- `created_at` 创建后永不修改 —— 这是 Append-only 架构的核心约束。
- 新节点创建时：`created_at = updated_at = now`。
- 强化时：仅更新 `updated_at`，不修改 `created_at`。
- 替换时：旧节点 `updated_at = now`，新节点 `created_at = updated_at = now`。
- 对历史记录（v1.0 之前写入）：缺失 `created_at` 或 `updated_at` 时值为 `null`，不影响系统运行。

**不引入的字段**（`transaction_time`, `valid_from`, `valid_to`, `ttl`, `decay`）：
当前系统为 State-based Memory（状态基记忆），尚未进入 Event Sourcing。这些字段需要事务支持或时态数据库能力，在未来的 v2.x 版本中考虑。
