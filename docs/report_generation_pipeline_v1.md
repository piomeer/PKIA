...
Scoring Pipeline（scoring_pipeline_schema_v1.md）的输出。
...
日报中的每一项推荐都必须附带 Reasoning。Section B 需要详细解释"为什么推荐"和"与职业目标的关系"。Section C 和 D 至少需要一句话说明。这既是 prompt_scoring_agent_v1.md 的强制要求，也是用户信任评分系统的前提。
...
| **Project Data Schema（规划中）** | 定义每个项目的标准化数据结构。报告生成管线的 Stage 5（Assembly）需要依赖 Schema 定义的数据字段进行格式化输出 |
| **scoring_pipeline_schema_v1.md** | 本文件的上游。Scoring Pipeline 负责"项目 → 分数"，Report Generation Pipeline 负责"分数 → 日报"。Score Pipeline 的输出（Project Name、Category、4 维度分数、Total Score、Recommendation、Reasoning）是本文件的全部输入 |
| **prompt_scoring_agent_v1.md** | 提供 Scoring Agent 的推理能力和 Reasoning 风格约束。报告生成管线不对 Reasoning 做重新生成，而是直接使用 Scoring Agent 输出的 Reasoning 原文。因此 prompt_scoring_agent_v1.md 中"禁止只输出分数"的要求间接影响了日报 Section B 的 Reasoning 质量 |
